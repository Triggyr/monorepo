...
export type ObjectPathsWithArray<Obj, KeyPrefix extends string = '', Depth extends number = 3> = Depth extends never ? never : true extends IsStrictlyAny<Obj> ? never : Obj extends Primitive | Date | AnyFunction ? never : {
    [K in keyof Unpack<Obj> & string]: Unpack<Obj>[K] extends Primitive ? `${KeyPrefix}${K}` : never | ObjectPathsWithArray<Unpack<Obj[K & keyof Obj]>, `${KeyPrefix}${K}${Obj[K & keyof Obj] extends AnyArray ? `` : ''}.`, [
        never,
        0,
        1,
        2,
...
export type ObjectPaths<Obj, KeyPrefix extends string = '', Depth extends number = 3> = Depth extends never ? never : true extends IsStrictlyAny<Obj> ? never : Obj extends Primitive | Date | AnyFunction ? never : {
...
export type PropertyValuexxx<Obj, Property extends Key> = Obj extends object ? Property extends `${infer Parent}.${infer Leaf}` ? Parent extends keyof Obj ? PropertyValuexxx<Obj[Parent], Leaf> : Parent extends `${infer SubParent}[${number}]` ? SubParent extends keyof Obj ? PropertyValuexxx<Unpack<Obj[SubParent]>, Leaf> : never : never : Property extends `${infer ArrayKey}[${number}]` ? ArrayKey extends keyof Obj ? Unpack<Obj[ArrayKey]> : never : Property extends keyof Obj ? Obj[Property] : never : never;
export {};
